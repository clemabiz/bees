...
class EntityEmbedDialog extends FormBase {
  ...
  public function buildSelectStep(array $form, FormStateInterface $form_state) {
    ...
    if ($this->entityBrowser) {
      ...
      if ($embed_button->getTypeSetting('inline_entity_form')
        && $this->moduleHandler->moduleExists('inline_entity_form')) {

        $form['entity_id']['#type'] = 'inline_entity_form';
        $form['entity_id']['#entity_type'] = $entity_element['data-entity-type'];
        $form['entity_id']['#bundle'] = reset($bundles);

        $entity_form_display = $embed_button->getTypePlugin()
          ->getConfigurationValue('inline_entity_form_settings')['entity_form_display'];

        $entity_form_display_name = $entity_element['data-entity-type'] . '.' . $form['entity_id']['#bundle']. '.' . $entity_form_display;
        $form_display = $this->entityTypeManager->getStorage('entity_form_display')
            ->load($entity_form_display_name);

        $form['entity_id']['entity_form_display'] = $entity_form_display_name;
        $form['entity_id']['mode'] = $entity_form_display_name;
  ...
  public function validateSelectStep(array &$form, FormStateInterface $form_state) {
    ...
    if ($form_state->hasValue(['entity_browser', 'entities'])) {
      ...
      if ($embed_button->getTypeSetting('inline_entity_form')
        ...
        InlineEntityForm::validateEntityForm($form['entity_id'], $form_state);
        if ($form_state->hasAnyErrors()) {
          return;
        }

        // Create a new entity to be embedded.
        InlineEntityForm::submitEntityForm($form['entity_id'], $form_state);

        if (isset($form['entity_id']['#entity'])) {
          /** @var \Drupal\Core\Entity\EntityInterface $ief_entity */
          $ief_entity = $form['entity_id']['#entity'];

          $entity_element = $form_state->get('entity_element');
          $entity_element['data-entity-uuid'] = $ief_entity->uuid();
          $form_state->set('entity_element', $entity_element);

          $entity_id = $ief_entity->Id();
        }
      }
  ...
  protected function loadInlineEntityForm(FormStateInterface $form_state) {
    ...
    if ($embed_button) {
      $this->inlineEntityForm = $embed_button->getTypePlugin()->getConfigurationValue('inline_entity_form');
